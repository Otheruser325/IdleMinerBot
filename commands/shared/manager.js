...
async function handleManagerHire(message, user, currentMine, userId, area) {
    ...
    if (managerExistsInMine(currentMine, newManager) || managerExistsInArea(currentMine, area, newManager)) {
        ...
        try {
            ...
            try {
                await (message.user || message.author).send(
...
async function handleManagerOverview(message, user, currentMine, area) {
    ...
    let currentPage = 0;
    const actorId = message.user?.id || message.author?.id;
    const customToken = `${actorId}_${Date.now()}`;
    ...
    const collector = trackComponentCollector(
        ...
        { ownerId: actorId, kind: 'manager-overview', ttlMs: 15 * 60 * 1000, idleMs: 60000 }
    );
    ...
    collector.on('collect', async interaction => {
        try {
            ...
            if (!await acknowledgeComponent(interaction, 'manager.collector')) return;
            ...
            logError('manager:overview:update', error, { userId: interaction?.user?.id, customId: interaction?.customId });
        }
    });
